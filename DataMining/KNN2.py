...
def selectPart(dataSet,cent,num):
    """
    :param dataSet: 数据
    :param cent: 质心
    :param num: 选取离质心的个数
    ...
    distancePart = []
    for index in sortedDistIndicies[:num]:
...
distancePartNum = 10
distancePart0 = selectPart(train_0,cent0.tolist()[0],distancePartNum)
distancePart1 = selectPart(train_1,cent1.tolist()[0],distancePartNum)
distancePart2 = selectPart(train_2,cent2.tolist()[0],distancePartNum)
print("distancePart0=",distancePart0,"\ndistancePart1=",distancePart1,"\ndistancePart2=",distancePart2)
centNext0=randCent(mat(distancePart0),1)
centNext1=randCent(mat(distancePart1),1)
centNext2=randCent(mat(distancePart2),1)
centTrainTotal = []
centLabels = []
centLabels.extend([0]+[1]+[2])
centTrainTotal.extend(centNext0.tolist()+centNext1.tolist()+centNext2.tolist())
# distancePartTrainTotal = []
# distancePartLabels = []
# distancePartLabels.extend([0 for _ in range(distancePartNum)]+[1 for _ in range(distancePartNum)]+[2 for _ in range(distancePartNum)])
...
print("trainingNum（经质心选取训练样本个数后计算的质心个数）=",array(centTrainTotal ).shape[0])
